...
public class CreateValidator
{
    ...
    public string? Validate(
        string dinoName, string dinoSpecies,
        string username,  string registerCode,
        int?   age)
    {
        ...
        if (string.IsNullOrWhiteSpace(dinoSpecies))
        ...
        // 4. Username único
        bool sobreNombreExists = _db.Dinosaurs.Any(d => d.Username == username);
        if (sobreNombreExists)
            return $"El identificador '{username}' ya está registrado en el sistema.";
